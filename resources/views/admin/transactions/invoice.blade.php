...
<head>
    ...
    <style>
        ...
        @media print {
            ...
            .invoice-container { margin: 0; box-shadow: none; width: 100%; height: 100%; border-radius: 0; }
...
<body class="antialiased">
    ...
    <div class="max-w-[210mm] mx-auto mt-6 mb-2 flex justify-between items-center no-print px-4">
        <div class="text-sm text-slate-500">Pratinjau Cetak Faktur. Margin disesuaikan untuk ukuran kertas A4.</div>
        <button onclick="window.print()" class="bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-700 hover:to-indigo-700 text-white px-6 py-2 rounded-xl font-semibold shadow-lg shadow-indigo-200 transition flex items-center gap-2">
    ...
    <!-- Invoice Paper -->
    <div class="invoice-container relative overflow-hidden">
        <!-- Top Colored Bar -->
        <div class="absolute top-0 left-0 w-full h-1.5 bg-gradient-to-r from-emerald-500 to-green-400 rounded-t-2xl"></div>

        <!-- Header Start -->
        <div class="flex justify-between items-start border-b border-slate-100 pb-8 mb-8 mt-4">
            <div>
                <h1 class="text-4xl font-extrabold text-green-600 tracking-tight">Techno<span class="text-slate-800">Market</span></h1>
                <p class="text-sm text-slate-500 mt-1">Platform Belanja Elektonik & Digital Terpercaya</p>
            ...
            <div class="text-right">
                <h2 class="text-3xl font-bold text-slate-200">INVOICE</h2>
                <p class="text-lg font-semibold text-slate-800 mt-1">#INV-{{ str_pad($transaction->id, 5, '0', STR_PAD_LEFT) }}</p>

                <div class="mt-4 text-sm grid grid-cols-2 gap-x-4 gap-y-2 text-right">
                    <div class="text-slate-500">Tanggal Transaksi:</div>
                    <div class="font-medium text-slate-800">{{ $transaction->created_at->format('d M Y, H:i') }}</div>
        ...
        <div class="grid grid-cols-2 gap-8 mb-8">
            <div class="bg-slate-50 p-5 rounded-xl border border-slate-100">
                <div class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">DITAGIHKAN KEPADA:</div>
                <h3 class="text-lg font-bold text-slate-800">{{ $transaction->buyer->name }}</h3>
        ...
        <!-- Payment Details -->
        <div class="mb-8">
            <h4 class="text-sm font-bold text-slate-800 mb-3 border-b border-slate-100 pb-2">Rincian Pembayaran & Pengiriman</h4>
            <div class="grid grid-cols-3 gap-4 text-sm">
                <div>
                    <span class="text-slate-500 block">Metode Pembayaran:</span>
                    <span class="font-semibold text-slate-800">{{ strtoupper($transaction->payment_method) }}</span>
                </div>
                <div>
                    <span class="text-slate-500 block">Kurir / Pengiriman:</span>
                    <span class="font-semibold text-slate-800">{{ strtoupper($transaction->courier ?? '-') }} / {{ strtoupper($transaction->delivery_type ?? '-') }}</span>
                </div>
                <div>
                    <span class="text-slate-500 block">Nomor Resi:</span>
                    <span class="font-mono font-semibold text-slate-800">{{ $transaction->tracking_number ?? 'Belum ada' }}</span>
                </div>
            </div>
        </div>

        <!-- Items Table -->
        <div class="mb-8 overflow-hidden border border-slate-200 rounded-xl">
            <table class="w-full text-left text-sm text-slate-600">
                <thead class="bg-slate-50 text-slate-700 text-xs uppercase border-b border-slate-200">
                    <tr>
                        ...
                        <th class="px-5 py-3 font-semibold text-right text-slate-900 bg-slate-100">Subtotal</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50/50">
                        <td class="px-5 py-4">
                            <div class="font-medium text-slate-800">{{ $item->product ? $item->product->name : 'Produk Tidak Ditetapkan' }}</div>
                        </td>
                        ...
                        <td class="px-5 py-4 text-right font-medium text-slate-800 bg-slate-50 border-l border-slate-100">Rp {{ number_format($item->price * $item->quantity, 0, ',', '.') }}</td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
            ...
            <div class="w-full md:w-1/2 lg:w-5/12 space-y-3 text-sm">
                ...
                <div class="pt-4 mt-2 border-t border-slate-200">
                    <div class="flex justify-between items-center">
                        ...
                        <span class="text-xl font-black text-green-600">Rp {{ number_format($transaction->total_amount, 0, ',', '.') }}</span>
                    </div>
                </div>
            </div>
        </div>
